...
set_client_classpath() {
  set_opendj_classpath bootstrap-client.jar
}

# Configure the appropriate CLASSPATH for server, using OpenDJ logger.
set_server_classpath() {
  set_opendj_classpath bootstrap.jar
}

# Configure the appropriate CLASSPATH for OpenDJ client or server, depending of the provided bootstrap jar file name.
set_opendj_classpath() {
  CLASSPATH="${INSTANCE_ROOT}/classes"
  CLASSPATH="${CLASSPATH}:${INSTALL_ROOT}/lib/${1}"
  for JAR in `ls ${INSTANCE_ROOT}/extlib/*.jar 2>/dev/null || true`
  do
