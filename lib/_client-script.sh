...
if test -x "${OPENDJ_NATIVE_BIN}"
then
  # Keep only system properties -Dxxx=yyy for native images
  OPENDJ_JAVA_ARGS_FOR_NATIVE=`echo ${OPENDJ_JAVA_ARGS} | sed -e 's/-[^D][^ ]*//g; s/\\s\+/ /g'`
  "${OPENDJ_NATIVE_BIN}" ${OPENDJ_JAVA_ARGS_FOR_NATIVE} ${SCRIPT_ARGS} ${SCRIPT_NAME_ARG} "${@}"
else
