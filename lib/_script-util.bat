...
rem Portions Copyright 2011-2017 ForgeRock AS.
...
FOR %%x in ("%INSTANCE_ROOT%\extlib\*.jar") DO call "%INSTALL_ROOT%\lib\setcp.bat" %%x
...
:readProperty
set PROPERTY_VALUE=
FOR /F "delims=" %%i IN ("%INSTANCE_ROOT%\config\java.properties") DO set JAVA_PROPERTIES=%%~dpnxi
if not exist "%JAVA_PROPERTIES%" goto:eof
set CMD=findstr /b [^#]*%~1=.* "%JAVA_PROPERTIES%"
for /f "tokens=2,* delims==" %%a in ( '%CMD%' ) do if "%%b"=="" (set PROPERTY_VALUE=%%a) else (set PROPERTY_VALUE=%%a=%%b)
goto:eof
